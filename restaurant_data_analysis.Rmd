...
i = sample(2, nrow(clean_data), replace = TRUE, prob = c(0.8, 0.2))

RestaurantTraining = clean_data[i==1, ] # 80% Training sample
RestaurantTest = clean_data[i==2, ] # 20% Testing sample
...
forward = stepAIC(null_model, direction='forward', scope=formula(full_model))
...
* Use the forward model to predict `rating` in `RestaurantTest` and calculate `MAE` and `MSE`.

```{r}

# Predicting rating
#ypred_forward = predict(object=forward, newdata = RestaurantTest)
