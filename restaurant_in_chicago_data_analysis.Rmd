...
# i = sample(2, nrow(data_to_split), replace = TRUE, prob = c(0.8, 0.2))

# RestaurantTraining = data_to_split[i==1, ] # 80% Training sample
# RestaurantTest = data_to_split[i==2, ] # 20% Testing sample

# # Check new dimensions
# cat("Training set size:", nrow(RestaurantTraining), "\n")
# cat("Testing set size:", nrow(RestaurantTest), "\n")

# colnames(RestaurantTraining)
# colnames(RestaurantTest)
```
...
# Extract the factor levels used by the model
# mr_levels = mr_model$xlevels$primaryType

# # Filter the testset to only include levels present in the training set
# RestaurantTest$primaryType = factor(RestaurantTest$primaryType, levels = mr_levels)
...
# mr_df <- data.frame(
#   y_true = RestaurantTest$rating,
#   y_pred = ypred
# ) %>% na.omit() # Remove rows with NA values in either column

# # mean absolute error of predicted rating, and Actual rating
# MAE(y_pred = mr_df$y_pred, y_true = RestaurantTest$rating)
MAE(y_pred = ypred , y_true = RestaurantTest$rating)

# # mean square error of predicted rating, and Actual rating
# MSE(y_pred = mr_df$y_pred, y_true = RestaurantTest$rating)
...
# Extract the factor levels used by the model
# model_levels = step_forward$xlevels$primaryType

# Filter the testset to only include levels present in the training set
# RestaurantTest$primaryType = factor(RestaurantTest$primaryType, levels = model_levels)
...
#   y_true = RestaurantTest$rating,
#   y_pred = ypred_forward
# ) %>% na.omit() # Remove rows with NA values in either column

# mean absolute error of predicted rating, and Actual rating
# MAE(y_pred = results_df$y_pred, y_true = results_df$y_true)
MAE(y_pred = ypred_forward, y_true = RestaurantTest$rating)

# mean square error of predicted rating, and Actual rating
# MSE(y_pred = results_df$y_pred, y_true = results_df$y_true)
